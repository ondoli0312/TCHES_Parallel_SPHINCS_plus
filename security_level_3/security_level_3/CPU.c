...
void CPU_set_tree_addr(uint32_t addr[8], uint64_t tree) {
    CPU_ull_to_bytes(&((uint8_t*)addr)[SPX_OFFSET_TREE], 8, tree);
}
...
void CPU_gen_message_random(uint8_t* sig, uint8_t* sk_prf, uint8_t* optrand, uint8_t* m, size_t mlen) {
    uint8_t buf[SPX_SHA256_BLOCK_BYTES + SPX_SHA256_OUTPUT_BYTES] = { 0, };
    uint8_t state[40];
    ...
    if (SPX_N + mlen < SPX_SHA256_BLOCK_BYTES) {
        ...
        CPU_hash_inc_finalize(buf + SPX_SHA256_BLOCK_BYTES, state, buf, mlen + SPX_N);
    }
    else {
        ...
        CPU_hash_inc_finalize(buf + SPX_SHA256_BLOCK_BYTES, state, m, mlen);
    }
    ...
    CPU_hash(buf, buf, SPX_SHA256_BLOCK_BYTES + SPX_SHA256_OUTPUT_BYTES);
    memcpy(sig, buf, SPX_N);
}
...
#define SPX_INBLOCKS (((SPX_N + SPX_PK_BYTES + SPX_SHA256_BLOCK_BYTES - 1) & \
    ...
    if (SPX_N + SPX_PK_BYTES + mlen < SPX_INBLOCKS * SPX_SHA256_BLOCK_BYTES) {
        ...
        CPU_hash_inc_finalize(seed, state, inbuf, SPX_N + SPX_PK_BYTES + mlen);
    }
    else {
        memcpy(inbuf + SPX_N + SPX_PK_BYTES, m, SPX_INBLOCKS * SPX_SHA256_BLOCK_BYTES - SPX_N - SPX_PK_BYTES);
    ...
    CPU_mgf1(bufp, SPX_DGST_BYTES, seed, SPX_SHA256_OUTPUT_BYTES);
